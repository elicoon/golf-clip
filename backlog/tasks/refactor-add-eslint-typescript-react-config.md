...
#### Acceptance Criteria
- [x] ESLint config exists at `apps/browser/.eslintrc.cjs` (or `eslint.config.js`) with `@typescript-eslint/eslint-plugin` and `eslint-plugin-react-hooks` enabled
- [x] `npm run lint` script works in browser workspace and exits 0 with no errors
- [x] CI workflow runs lint step (add to `.github/workflows/test.yml`)
- [x] Any auto-fixable violations are fixed; remaining violations documented with inline `eslint-disable` comments and justification
